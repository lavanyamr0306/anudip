...
public class ResumeController {
    ...
    public String analyze(@RequestParam(value = "file", required = false) MultipartFile file,
            ...
            @RequestParam(value = "resumeSource", defaultValue = "manual") String resumeSource,
        ...
        try {
            ...
            String normalizedResumeSource = resumeSource == null ? "manual" : resumeSource.trim().toLowerCase();

            Resume resume = null;
            String sourceFileName = "pasted-resume.txt";

            if ("profile".equals(normalizedResumeSource)) {
                UserProfile profile = userProfileService.getExistingProfile(user);
                if (profile == null || profile.getResumeText() == null || profile.getResumeText().isBlank()) {
                    populateDashboardModel(model, user, service.getLatestAnalysis(user));
                    model.addAttribute("error",
                            "No resume was found in your profile. Please complete your profile and upload a resume first.");
                    model.addAttribute("resumeSource", normalizedResumeSource);
                    return "dashboard";
                }
                normalizedResumeContent = profile.getResumeText().trim();
                sourceFileName = profile.getResumeFileName() == null || profile.getResumeFileName().isBlank()
                        ? "profile-resume"
            ...
            model.addAttribute("sourceFileName", resume != null ? resume.getFileName() : sourceFileName);
            model.addAttribute("resumeSource", normalizedResumeSource);
            return "dashboard";
            ...
            model.addAttribute("resumeSource", resumeSource == null ? "manual" : resumeSource);
            return "dashboard";
        }
    }
    ...
    @GetMapping("/analysis/{id}/job-details")
    public String viewJobDetails(@PathVariable Long id,
        ...
        if (analysis.getRecommendedJobs() == null || !analysis.getRecommendedJobs().contains(jobRole)) {
            populateDashboardModel(model, user, analysis);
            model.addAttribute("error", "Selected job is not available in this analysis.");
            return "dashboard";
        }

        JobDetail jobDetail = service.getJobDetailByRole(jobRole);
        if (jobDetail == null) {
            populateDashboardModel(model, user, analysis);
            model.addAttribute("error", "Job details not found for the selected role.");
            return "dashboard";
        }
        ...
        model.addAttribute("jobSkills", service.splitSkills(jobDetail.getSkillsRequired()));
    ...
    private void populateDashboardModel(Model model, User user, Analysis selectedAnalysis) {
        ...
        model.addAttribute("profileResumeAvailable", profile.hasResume());
